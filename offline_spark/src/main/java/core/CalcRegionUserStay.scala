...
object CalcRegionUserStay {
  def main(args: Array[String]): Unit = {
    ...
    //加载基站信令数据-HDFS
    val laccellSignFilePath = "hdfs://bigdata01:9000/data/laccellSign/20221208/11"
    ...
    resDf.rdd.foreachPartition(it=>{
      ...
      it.foreach(row=>{
        val region_id = row.getString(0)
        val produce_hour = row.getString(1)
